...
def detectPose(image, pose, display=True):
    ...
    if results.pose_landmarks and results.pose_landmarks.landmark[28].visibility > 0.6 and results.pose_landmarks.landmark[27].visibility > 0.6 and results.pose_landmarks.landmark[15].visibility>0.6 and results.pose_landmarks.landmark[16].visibility > 0.6:
        ...
        cv2.putText(output_image, "Make Sure Full body visible", (320,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255),2)
...
def classifyPose(landmarks, output_image, display=False):
    ...
    #Get coordinates.
    left_elbow = list(landmarks[mp_pose.PoseLandmark.LEFT_ELBOW.value])
    right_elbow = list(landmarks[mp_pose.PoseLandmark.RIGHT_ELBOW.value])
    left_shoulder = list(landmarks[mp_pose.PoseLandmark.LEFT_SHOULDER.value])
    right_shoulder = list(landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value])
    left_hip = list(landmarks[mp_pose.PoseLandmark.LEFT_HIP.value])
    right_hip = list(landmarks[mp_pose.PoseLandmark.RIGHT_HIP.value])
    left_knee = list(landmarks[mp_pose.PoseLandmark.LEFT_KNEE.value])
    right_knee = list(landmarks[mp_pose.PoseLandmark.RIGHT_KNEE.value])
